...
function Login() {
  ...
  const onSubmit = React.useCallback(async (props: any) => {
    try {
      const bearerToken = await axios.post(`http://${PRIVATE_IP_ADDRESS}:3000/login`, props)
      await AsyncStorage.setItem('@bearerToken', bearerToken.data.access_token)
      router.push('/')
    } catch (error) {
      setError(true)
    }
          ...
          <YStack width={Dimensions.get('window').width - 30} space={'$8'}>
            {formError ? (
              ...
              <>
                ...
                <Paragraph onPress={() => router.push('/register')}>
